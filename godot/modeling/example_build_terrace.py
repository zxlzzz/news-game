# 范例（2026-09-19 画风测试时写的，原在 sth/godot-npc/tools/build_terrace.py）。
# 用途：示范"用脚本拼出合规几何"的写法——box / revolve / extrude + manifold 布尔、按色槽分组、
# 材质名 = 色槽名、平面着色、导出 glb。依赖 pip: trimesh manifold3d shapely。
# 注意它和正式交付的区别：它是一整个场景而不是单个物件，原点不在接地点（下层地面在 y=-4.2），
# 材质里还写了颜色（现在颜色由配色文件决定，材质只需要名字），导出路径是当时容器里的路径。
