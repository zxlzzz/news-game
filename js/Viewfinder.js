...
export class Viewfinder {
  ...
  draw(g) {
    ...
    g.moveTo(cx,      cy     ).lineTo(cx + cornerLen, cy);
    g.moveTo(cx,      cy     ).lineTo(cx,      cy + cornerLen);
    g.moveTo(cx + cw, cy     ).lineTo(cx + cw - cornerLen, cy);
    g.moveTo(cx + cw, cy     ).lineTo(cx + cw, cy + cornerLen);
    g.moveTo(cx,      cy + ch).lineTo(cx + cornerLen,      cy + ch);
    g.moveTo(cx,      cy + ch).lineTo(cx,      cy + ch - cornerLen);
    g.moveTo(cx + cw, cy + ch).lineTo(cx + cw - cornerLen, cy + ch);
    g.moveTo(cx + cw, cy + ch).lineTo(cx + cw, cy + ch - cornerLen);

    // 中心十字
    const mx = cx + cw / 2, my = cy + ch / 2;
    g.lineStyle(1, 0xffffff, 0.3);
    g.moveTo(mx - 10, my).lineTo(mx + 10, my);
    g.moveTo(mx, my - 10).lineTo(mx, my + 10);
  ...
  _drawResizeHandle(g, hx, hy) {
    ...
    for (let i = 0; i < 3; i++) {
      const off = 3 + i * 4;
      g.moveTo(hx - off, hy + 1).lineTo(hx + 1, hy - off);
    }
  }
