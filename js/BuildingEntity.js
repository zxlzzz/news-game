/**
 * BuildingEntity — 俯视角"老街区"沿街楼，6 种原型：
 *   resi        普通居民楼（主）：6–8 层，浅灰墙，密集小阳台+外挂空调+晾衣，底层便利店/小餐馆
 *   oldmix      老旧商住楼：4–6 层，偏窄，防盗网+卷帘门+不规则招牌，墙面略脏，拥挤感
 *   modern      小型现代商业楼：3–5 层，玻璃立面，干净，底层咖啡/书店/健身
 *   clinic      社区诊所：2–3 层，白灰，规则窗，门口小雨棚+十字
 *   convenience 街角便利店：矮，横向大招牌，玻璃门，门口堆货箱+自动售货机
 *   bookstore   老式书店：窄门面，木质招牌，橱窗摆书，略旧文艺
 *
 * 纯灰阶。高度可由 scene.json 的 facadeH 覆盖（天际线参差）；可选 waterTower/solar/billboard。
 *
 * 坐标：this.x=左边缘；this.y=立面顶边；底边=this.y+facadeH=BUILDING_BASE_Y；屋顶在 y 之上。
 */

import { Entity } from './Entity.js';
import { LINE_FAR_COLOR, LINE_FAR_WIDTH } from './SceneConfig.js';

// 原型参数表
const ARCH = {
  resi: {
    wall: 0xc6c6c6, roof: 0xb6b6b6, floorH: 15, groundFrac: 0.24, groundMax: 26,
    style: 'windows', balcony: true, grille: false, glass: false,
    dirty: 0.0, laundry: 0.4, acFreq: 0.45, ground: 'shop', shops: ['convenience', 'fork'],
  },
  oldmix: {
    wall: 0x9c9c9c, roof: 0x8c8c8c, floorH: 16, groundFrac: 0.30, groundMax: 30,
    style: 'grille', balcony: false, grille: true, glass: false,
    dirty: 0.55, laundry: 0.5, acFreq: 0.6, ground: 'roller', shops: ['fork', 'dots'],
  },
  modern: {
    wall: 0xdadada, roof: 0xcacaca, floorH: 16, groundFrac: 0.26, groundMax: 26,
    style: 'glass', balcony: false, grille: false, glass: true,
    dirty: 0.0, laundry: 0.0, acFreq: 0.1, ground: 'glassshop', shops: ['cup', 'book', 'dumbbell'],
  },
  clinic: {
    wall: 0xe2e2e2, roof: 0xd2d2d2, floorH: 15, groundFrac: 0.34, groundMax: 24,
    style: 'windows', balcony: false, grille: false, glass: false,
    dirty: 0.0, laundry: 0.0, acFreq: 0.15, ground: 'clinic', shops: ['cross'],
  },
  convenience: {
    wall: 0xcfcfcf, roof: 0xc0c0c0, floorH: 14, groundFrac: 0.58, groundMax: 28,
    style: 'windows', balcony: false, grille: false, glass: false,
    dirty: 0.1, laundry: 0.0, acFreq: 0.3, ground: 'cvs', shops: ['dots'],
  },
  bookstore: {
    wall: 0xbcbcbc, roof: 0xacacac, floorH: 15, groundFrac: 0.40, groundMax: 26,
    style: 'windows', balcony: false, grille: false, glass: false,
    dirty: 0.25, laundry: 0.0, acFreq: 0.2, ground: 'bookshop', shops: ['book'],
  },
  default: {
    wall: 0xc6c6c6, roof: 0xb6b6b6, floorH: 15, groundFrac: 0.26, groundMax: 26,
    style: 'windows', balcony: true, grille: false, glass: false,
    dirty: 0.1, laundry: 0.3, acFreq: 0.4, ground: 'shop', shops: ['dots'],
  },
};

function seededRand(x, salt = 0) {
  const s = Math.sin(x * 12.9898 + salt * 78.233) * 43758.5453;
  return s - Math.floor(s);
}

// Helper: fill rect
function fr(g, c, a, rx, ry, rw, rh) {
  g.lineStyle(0);
  g.beginFill(c, a);
  g.drawRect(rx, ry, rw, rh);
  g.endFill();
}

// Helper: stroke rect (lineStyle must be set before calling)
function sr(g, rx, ry, rw, rh) {
  g.endFill();
  g.drawRect(rx, ry, rw, rh);
}

// Helper: line
function ln(g, x1, y1, x2, y2) {
  g.moveTo(x1, y1).lineTo(x2, y2);
}

export class BuildingEntity extends Entity {
  constructor(config) {
    super({ ...config, width: config.bWidth ?? 100, height: config.bDepth ?? 70, static: true });
    this.bWidth     = config.bWidth     ?? 100;
    this.bDepth     = config.bDepth     ?? 70;
    this.waterTower = config.waterTower ?? false;
    this.solar      = config.solar      ?? false;
    this.billboard  = config.billboard  ?? false;

    const t = (this.tags ?? []).find(tag => tag in ARCH) || 'default';
    this.arch = t;
    this.A    = ARCH[t];
    this.facadeH = config.facadeH ?? 90;
  }

  get _sortY() { return this.y + this.facadeH; }

  getBounds() {
    return { x: this.x, y: this.y - this.bDepth, width: this.bWidth, height: this.bDepth };
  }

  draw(g) {
    if (!this.visible) return;
    const { x, bWidth: w, bDepth: d, A } = this;
    const base = this.y + this.facadeH;
    const top  = this.y - d;

    // ── 屋顶 ──
    fr(g, A.roof, 1, x, top, w, d);
    fr(g, 0xffffff, 0.10, x, top, w, Math.floor(d * 0.32));
    fr(g, 0x000000, 0.10, x, top + d * 0.68, w, d * 0.32);
    g.lineStyle(LINE_FAR_WIDTH, LINE_FAR_COLOR, 0.95); sr(g, x, top, w, d);
    this._roofDetails(g, top, d);
    if (this.solar)      this._solarPanels(g, top, d);
    if (this.waterTower) this._waterTower(g, top, d);
    if (this.billboard)  this._billboard(g, top);

    // 临街地面线
    g.lineStyle(1.2, 0x303030, 0.45); ln(g, x, base, x + w, base);

    // ── 立面 ──
    this._facade(g, x, w);

    // 巷道暗缝
    if (this.alleyLeft) {
      fr(g, 0x555555, 0.5, x - 1, top, 8, base - top + 1);
      fr(g, 0xffffff, 0.08, x + 7, top, 1.5, base - top + 1);
    }
  }

  _facade(g, x, w) {
    const H = this.facadeH, y = this.y, A = this.A;
    fr(g, A.wall, 1, x, y, w, H);
    fr(g, 0x000000, 0.16, x, y, 3, H);
    fr(g, 0x000000, 0.20, x, y, w, 3);

    const groundH = Math.min(A.groundMax, Math.round(H * A.groundFrac));
    const resH    = H - groundH;

    if (A.dirty > 0) this._dirty(g, x, y, w, H, A.dirty);

    if (A.style === 'glass')       this._upperGlass(g, x, y, w, resH);
    else if (A.style === 'grille') this._upperGrille(g, x, y, w, resH);
    else                           this._upperWindows(g, x, y, w, resH);

    this._ground(g, x, y + resH, w, groundH);

    g.lineStyle(LINE_FAR_WIDTH, LINE_FAR_COLOR, 0.95); sr(g, x, y, w, H);
  }

  _upperWindows(g, x, y, w, resH) {
    if (resH < 8) return;
    const A = this.A;
    const n  = Math.max(1, Math.round(resH / A.floorH));
    const fh = resH / n;
    const colW = 10, gap = 7;
    const nCol = Math.max(1, Math.floor((w - 6) / (colW + gap)));
    const sx = x + (w - (nCol * (colW + gap) - gap)) / 2;

    for (let f = 0; f < n; f++) {
      const fy = y + f * fh;
      g.lineStyle(0.6, 0x2c2c2c, 0.45); ln(g, x + 2, fy, x + w - 2, fy);
      for (let c = 0; c < nCol; c++) {
        const wx = sx + c * (colW + gap), wy = fy + 2, wh = fh - 6;
        if (wh < 3) continue;
        const v = seededRand(x + wx * 1.3, f * 3 + c);
        const shade = v > 0.78 ? 0x9a9a9a : (v < 0.2 ? 0x2a2a2a : 0x383838);
        fr(g, shade, 0.85, wx, wy, colW, wh);
        g.lineStyle(0.4, 0xb8b8b8, 0.45); ln(g, wx + colW / 2, wy, wx + colW / 2, wy + wh);
        g.lineStyle(0.5, 0x101010, 0.8); sr(g, wx, wy, colW, wh);
        if (A.balcony) {
          const ry = fy + fh - 3;
          g.lineStyle(0.6, 0x555555, 0.85); ln(g, wx - 1, ry, wx + colW + 1, ry);
          for (let bx = wx; bx <= wx + colW; bx += 3) ln(g, bx, ry, bx, ry + 2.5);
        }
        const dec = seededRand(x + wx, f * 7 + c * 5);
        if (A.acFreq && dec < A.acFreq && wx + colW + 6 < x + w) this._ac(g, wx + colW + 1, wy + 1);
        else if (A.laundry && dec > 1 - A.laundry) this._laundry(g, wx, colW, wy);
        else if (A.balcony && dec > 0.6 && dec < 0.72) this._green(g, wx + colW / 2, fy + fh - 3);
      }
    }
  }

  _upperGrille(g, x, y, w, resH) {
    if (resH < 8) return;
    const A = this.A;
    const n  = Math.max(1, Math.round(resH / A.floorH));
    const fh = resH / n;
    const colW = 9, gap = 5;
    const nCol = Math.max(1, Math.floor((w - 4) / (colW + gap)));
    const sx = x + (w - (nCol * (colW + gap) - gap)) / 2;
    for (let f = 0; f < n; f++) {
      const fy = y + f * fh;
      g.lineStyle(0.6, 0x202020, 0.5); ln(g, x + 2, fy, x + w - 2, fy);
      for (let c = 0; c < nCol; c++) {
        const jitter = (seededRand(x + c * 9, f) - 0.5) * 2;
        const wx = sx + c * (colW + gap), wy = fy + 2 + jitter, wh = fh - 6;
        if (wh < 3) continue;
        fr(g, 0x303030, 0.85, wx, wy, colW, wh);
        g.lineStyle(0.5, 0x101010, 0.85); sr(g, wx, wy, colW, wh);
        g.lineStyle(0.4, 0xbcbcbc, 0.7); sr(g, wx - 1, wy - 1, colW + 2, wh + 2);
        for (let gx = wx + 2; gx < wx + colW; gx += 3) ln(g, gx, wy - 1, gx, wy + wh + 1);
        for (let gyv = wy + 2; gyv < wy + wh; gyv += 3) ln(g, wx - 1, gyv, wx + colW + 1, gyv);
        const dec = seededRand(x + wx, f * 4 + c);
        if (dec < A.acFreq && wx + colW + 6 < x + w) this._ac(g, wx + colW + 1, wy + 1);
        else if (dec > 1 - A.laundry) this._laundry(g, wx, colW, wy);
      }
    }
  }

  _upperGlass(g, x, y, w, resH) {
    if (resH < 8) return;
    const A = this.A;
    const n  = Math.max(2, Math.round(resH / A.floorH));
    const fh = resH / n;
    for (let f = 0; f < n; f++) {
      const fy = y + f * fh + 1.5;
      const bh = fh - 3;
      if (bh < 2) continue;
      fr(g, 0x444444, 0.5, x + 4, fy, w - 8, bh);
      fr(g, 0xffffff, 0.18, x + 4, fy, w - 8, bh * 0.45);
      g.lineStyle(0.5, 0x101010, 0.75); sr(g, x + 4, fy, w - 8, bh);
      const segs = Math.max(3, Math.floor((w - 8) / 12));
      g.lineStyle(0.4, 0x9a9a9a, 0.5);
      for (let i = 1; i < segs; i++) { const lx = x + 4 + (w - 8) * i / segs; ln(g, lx, fy, lx, fy + bh); }
    }
  }

  _dirty(g, x, y, w, H, amount) {
    const k = Math.floor(amount * 5);
    for (let i = 0; i < k; i++) {
      const dx = x + 3 + seededRand(x, 100 + i) * (w - 8);
      const dw = 1 + seededRand(x, 110 + i) * 2.5;
      fr(g, 0x000000, 0.07 + seededRand(x, 120 + i) * 0.06,
         dx, y + 2, dw, H * (0.4 + seededRand(x, 130 + i) * 0.5));
    }
  }

  _ground(g, x, gy, w, gh) {
    switch (this.A.ground) {
      case 'roller':    this._gRoller(g, x, gy, w, gh); break;
      case 'glassshop': this._gGlassShop(g, x, gy, w, gh); break;
      case 'clinic':    this._gClinic(g, x, gy, w, gh); break;
      case 'cvs':       this._gCvs(g, x, gy, w, gh); break;
      case 'bookshop':  this._gBookshop(g, x, gy, w, gh); break;
      case 'shop':
      default:          this._gShop(g, x, gy, w, gh); break;
    }
  }

  _shopKind() { const s = this.A.shops; return s[Math.floor(seededRand(this.x, 7) * s.length)]; }
  _signH(min, max) { return Math.round(min + seededRand(this.x, 3) * (max - min)); }

  _gShop(g, x, gy, w, gh) {
    const sh = this._signH(5, 8);
    fr(g, 0x8a8a8a, 1, x + 3, gy + 1, w - 6, 3);
    g.lineStyle(0.5, 0x101010, 0.8); sr(g, x + 3, gy + 1, w - 6, 3);
    fr(g, 0x2a2a2a, 1, x + 4, gy + 4, w - 8, sh);
    this._icon(g, x + w / 2, gy + 4 + sh / 2, this._shopKind());
    this._shopGlass(g, x, gy + 4 + sh, w, gh - (4 + sh));
  }

  _gRoller(g, x, gy, w, gh) {
    const sh = this._signH(5, 9);
    const off = (seededRand(x, 4) - 0.5) * 4;
    fr(g, 0x1f1f1f, 1, x + 4 + off, gy + 2, w - 12, sh);
    g.lineStyle(0.5, 0x000000, 0.9); sr(g, x + 4 + off, gy + 2, w - 12, sh);
    this._icon(g, x + w / 2 + off, gy + 2 + sh / 2, this._shopKind());
    const rY = gy + 3 + sh, rH = gh - (3 + sh) - 1;
    if (rH > 2) {
      fr(g, 0x6e6e6e, 1, x + 4, rY, w - 8, rH);
      g.lineStyle(0.4, 0x3a3a3a, 0.8);
      for (let ly = rY + 1.5; ly < rY + rH; ly += 2) ln(g, x + 4, ly, x + w - 4, ly);
      g.lineStyle(0.5, 0x101010, 0.85); sr(g, x + 4, rY, w - 8, rH);
    }
  }

  _gGlassShop(g, x, gy, w, gh) {
    fr(g, 0x2f2f2f, 1, x + 4, gy + 1, w - 8, 4);
    this._icon(g, x + w / 2, gy + 3, this._shopKind());
    const glY = gy + 6, glH = gh - 7;
    if (glH > 2) {
      fr(g, 0x3a3a3a, 0.5, x + 4, glY, w - 8, glH);
      fr(g, 0xffffff, 0.2, x + 5, glY + 1, w - 10, glH / 2);
      g.lineStyle(0.6, 0x101010, 0.8); sr(g, x + 4, glY, w - 8, glH);
      const segs = Math.max(2, Math.floor((w - 8) / 16));
      g.lineStyle(0.4, 0x808080, 0.5);
      for (let i = 1; i < segs; i++) { const lx = x + 4 + (w - 8) * i / segs; ln(g, lx, glY, lx, glY + glH); }
    }
  }

  _gClinic(g, x, gy, w, gh) {
    this._shopGlass(g, x, gy + 6, w, gh - 6, /*lite=*/true);
    fr(g, 0x2a2a2a, 1, x + 4, gy + 1, w - 8, 5);
    this._icon(g, x + w / 2, gy + 3.5, 'cross');
    const eW = Math.min(26, w * 0.4), eX = x + (w - eW) / 2;
    fr(g, 0x5a5a5a, 1, eX - 2, gy + 6, eW + 4, 3);
    g.lineStyle(0.5, 0x101010, 0.85); sr(g, eX - 2, gy + 6, eW + 4, 3);
    g.lineStyle(0.6, 0x707070, 0.8); ln(g, x + 6, gy + gh, x + w - 6, gy + gh);
  }

  _gCvs(g, x, gy, w, gh) {
    fr(g, 0x232323, 1, x + 2, gy + 1, w - 4, 7);
    g.lineStyle(0.5, 0x000000, 0.9); sr(g, x + 2, gy + 1, w - 4, 7);
    this._icon(g, x + w / 2, gy + 4.5, 'dots');
    const glY = gy + 9, glH = gh - 10;
    if (glH > 2) {
      fr(g, 0x383838, 0.55, x + 4, glY, w - 8, glH);
      fr(g, 0xffffff, 0.2, x + 5, glY + 1, w - 10, glH / 2);
      g.lineStyle(0.6, 0x101010, 0.8); sr(g, x + 4, glY, w - 8, glH);
      fr(g, 0x9a9a9a, 1, x + 5, glY + glH - 4, 6, 4);
      fr(g, 0x9a9a9a, 1, x + 12, glY + glH - 3, 5, 3);
      g.lineStyle(0.4, 0x303030, 0.85);
      sr(g, x + 5, glY + glH - 4, 6, 4); sr(g, x + 12, glY + glH - 3, 5, 3);
      const vW = 8, vX = x + w - 6 - vW;
      fr(g, 0x4a4a4a, 1, vX, glY + 1, vW, glH - 1);
      g.lineStyle(0.4, 0xcacaca, 0.6); sr(g, vX + 1, glY + 2, vW - 4, glH - 4);
      g.lineStyle(0.5, 0x101010, 0.85); sr(g, vX, glY + 1, vW, glH - 1);
    }
  }

  _gBookshop(g, x, gy, w, gh) {
    fr(g, 0x4a4a4a, 1, x + 3, gy + 1, w - 6, 6);
    g.lineStyle(0.6, 0x202020, 0.9); sr(g, x + 3, gy + 1, w - 6, 6);
    this._icon(g, x + w / 2, gy + 4, 'book');
    const glY = gy + 8, glH = gh - 9;
    if (glH > 2) {
      fr(g, 0x383838, 0.5, x + 4, glY, w - 8, glH);
      g.lineStyle(0.6, 0x101010, 0.8); sr(g, x + 4, glY, w - 8, glH);
      let bx = x + 6;
      while (bx < x + w - 10) {
        const bw = 1.5 + seededRand(x + bx, 1) * 2;
        const bhh = glH * (0.4 + seededRand(x + bx, 2) * 0.5);
        const sh = 0x6a6a6a + Math.floor(seededRand(x + bx, 3) * 0x40) * 0x010101;
        fr(g, sh, 0.9, bx, glY + glH - bhh, bw, bhh);
        bx += bw + 1.2;
      }
      fr(g, 0x1a1a1a, 0.9, x + w - 5 - 7, glY + 1, 7, glH - 1);
    }
  }

  _shopGlass(g, x, gy, w, gh, lite = false) {
    if (gh <= 2) return;
    fr(g, lite ? 0x4a4a4a : 0x383838, lite ? 0.45 : 0.55, x + 4, gy, w - 8, gh);
    fr(g, 0xffffff, lite ? 0.22 : 0.18, x + 5, gy + 1, w - 10, gh / 2);
    g.lineStyle(0.6, 0x101010, 0.8); sr(g, x + 4, gy, w - 8, gh);
    const segs = Math.max(2, Math.floor((w - 8) / 18));
    g.lineStyle(0.4, 0x808080, 0.5);
    for (let i = 1; i < segs; i++) { const lx = x + 4 + (w - 8) * i / segs; ln(g, lx, gy, lx, gy + gh); }
    const dW = 9;
    fr(g, 0x1a1a1a, 0.92, x + w - 6 - dW, gy + 1, dW, gh - 1);
    g.lineStyle(0.5, 0x000000, 0.9); sr(g, x + w - 6 - dW, gy + 1, dW, gh - 1);
  }

  _ac(g, ax, ay) {
    fr(g, 0x9a9a9a, 1, ax, ay, 4, 3);
    g.lineStyle(0.4, 0x303030, 0.85); sr(g, ax, ay, 4, 3);
    ln(g, ax + 0.5, ay + 1.5, ax + 3.5, ay + 1.5);
  }

  _laundry(g, wx, colW, wy) {
    const ly = wy - 1;
    g.lineStyle(0.4, 0x707070, 0.8); ln(g, wx, ly, wx + colW, ly);
    fr(g, 0xcacaca, 0.85, wx + 2, ly, 2, 4);
    fr(g, 0xcacaca, 0.85, wx + 6, ly, 2, 3);
  }

  _green(g, cx, ry) {
    fr(g, 0x666666, 1, cx - 3, ry - 2, 6, 2);
    fr(g, 0x9c9c9c, 0.95, cx - 2.5, ry - 4, 2, 2);
    fr(g, 0x9c9c9c, 0.95, cx + 0.5, ry - 4, 2, 2);
  }

  _icon(g, cx, cy, type) {
    g.lineStyle(0.8, 0xe8e8e8, 0.95);
    switch (type) {
      case 'cart':
        ln(g, cx - 4, cy - 2, cx + 3, cy - 2); ln(g, cx - 2, cy - 2, cx - 1, cy + 1);
        ln(g, cx - 1, cy + 1, cx + 3, cy + 1);
        fr(g, 0xe8e8e8, 0.95, cx - 1, cy + 2, 1.2, 1.2);
        fr(g, 0xe8e8e8, 0.95, cx + 2, cy + 2, 1.2, 1.2); break;
      case 'cup':
        ln(g, cx - 3, cy - 2, cx + 2, cy - 2); ln(g, cx - 3, cy - 2, cx - 2, cy + 2);
        ln(g, cx + 2, cy - 2, cx + 1, cy + 2); ln(g, cx - 2, cy + 2, cx + 1, cy + 2);
        ln(g, cx + 2, cy - 1, cx + 4, cy); break;
      case 'dumbbell':
        ln(g, cx - 3, cy, cx + 3, cy);
        fr(g, 0xe8e8e8, 0.95, cx - 4, cy - 2, 1.5, 4);
        fr(g, 0xe8e8e8, 0.95, cx + 2.5, cy - 2, 1.5, 4); break;
      case 'book':
        g.lineStyle(0.8, 0xe8e8e8, 0.95); sr(g, cx - 4, cy - 2.5, 8, 5);
        g.lineStyle(0.8, 0xe8e8e8, 0.95); ln(g, cx, cy - 2.5, cx, cy + 2.5); break;
      case 'burger':
        ln(g, cx - 4, cy - 2, cx + 4, cy - 2); ln(g, cx - 4, cy, cx + 4, cy); ln(g, cx - 4, cy + 2, cx + 4, cy + 2); break;
      case 'cross':
        ln(g, cx, cy - 3, cx, cy + 3); ln(g, cx - 3, cy, cx + 3, cy); break;
      case 'fork':
        ln(g, cx - 2, cy - 3, cx - 2, cy + 3); ln(g, cx + 2, cy - 3, cx + 2, cy + 3); break;
      case 'dots':
      default:
        for (const dx of [-3, 0, 3]) fr(g, 0xe8e8e8, 0.95, cx + dx - 0.7, cy - 0.7, 1.4, 1.4); break;
    }
  }

  _roofDetails(g, roofTop, d) {
    const x = this.x, w = this.bWidth;
    const acCount = Math.floor(this.A.acFreq * 4);
    for (let i = 0; i < acCount; i++) {
      const ax = x + 8 + seededRand(x, 10 + i) * (w - 20);
      const ay = roofTop + 6 + seededRand(x, 20 + i) * (d - 16);
      const aw = 6 + Math.floor(seededRand(x, 30 + i) * 5), ah = 4 + Math.floor(seededRand(x, 40 + i) * 3);
      fr(g, 0x9a9a9a, 1, ax, ay, aw, ah);
      g.lineStyle(0.6, 0x303030, 0.85); sr(g, ax, ay, aw, ah);
    }
    if (seededRand(x, 50) > 0.5) {
      const ax = x + w * (0.5 + (seededRand(x, 51) - 0.5) * 0.5), ay = roofTop + d * 0.3;
      g.lineStyle(0.7, 0x202020, 0.95);
      ln(g, ax, ay, ax, ay - 12);
      ln(g, ax - 3, ay - 4, ax + 3, ay - 4); ln(g, ax - 2, ay - 7, ax + 2, ay - 7);
    }
  }

  _solarPanels(g, roofTop, d) {
    const x = this.x, w = this.bWidth;
    const px = x + w * 0.5, py = roofTop + d * 0.4;
    for (let i = 0; i < 2; i++) {
      const sx = px - 14 + i * 15, sy = py - 3;
      fr(g, 0x404040, 0.95, sx, sy, 13, 7);
      g.lineStyle(0.4, 0x9a9a9a, 0.7);
      for (let k = 1; k < 3; k++) ln(g, sx + 13 * k / 3, sy, sx + 13 * k / 3, sy + 7);
      ln(g, sx, sy + 3.5, sx + 13, sy + 3.5);
      g.lineStyle(0.5, 0x101010, 0.85); sr(g, sx, sy, 13, 7);
    }
  }

  _billboard(g, roofTop) {
    const x = this.x, w = this.bWidth;
    const bw = Math.min(40, w * 0.5), bx = x + (w - bw) / 2, by = roofTop - 16;
    g.lineStyle(0.7, 0x303030, 0.9);
    ln(g, bx + 4, roofTop, bx + 4, by + 10); ln(g, bx + bw - 4, roofTop, bx + bw - 4, by + 10);
    fr(g, 0xcfcfcf, 1, bx, by, bw, 10);
    g.lineStyle(0.6, 0x202020, 0.9); sr(g, bx, by, bw, 10);
    g.lineStyle(0.5, 0x9a9a9a, 0.7);
    ln(g, bx + 4, by + 4, bx + bw - 4, by + 4); ln(g, bx + 4, by + 6.5, bx + bw - 10, by + 6.5);
  }

  _waterTower(g, roofTop, d) {
    const wx = this.x + this.bWidth * 0.32, wy = roofTop + d * 0.3, sz = 7;
    fr(g, 0xb8b8b8, 1, wx - sz, wy - sz, sz * 2, sz * 2);
    g.lineStyle(0.7, 0x303030, 0.95); sr(g, wx - sz, wy - sz, sz * 2, sz * 2);
    ln(g, wx - sz, wy - sz, wx, wy - sz - 5); ln(g, wx + sz, wy - sz, wx, wy - sz - 5);
    g.lineStyle(0.6, 0x303030, 0.85);
    ln(g, wx - sz + 2, wy + sz, wx - sz + 2, wy + sz + 5);
    ln(g, wx + sz - 2, wy + sz, wx + sz - 2, wy + sz + 5);
  }
}
