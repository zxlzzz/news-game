...
export class VehicleEntity extends Entity {
  ...
  _carWindows(g, cx, baseY, halfL, hs, d, sw) {
    ...
    g.moveTo(pillarX, winTopR - hs * 0.02).lineTo(pillarX, winBot);

    // 门缝线
    g.lineStyle(Math.max(0.5, sw * 0.3), 0x2a2a2a, 0.25);
    g.moveTo(pillarX, winBot).lineTo(pillarX, baseY + hs * 0.02);
  }
  ...
  _taxi(g, highlight) {
    ...
    for (let i = 0; i < checkN; i++) {
      g.beginFill(i % 2 === 0 ? 0x222222 : 0xe0e0e0, 0.8);
  ...
  _bus(g, highlight) {
    ...
    if (this.facingSide !== 'far') {
      ...
      if (this.doorOpen) {
        ...
        g.moveTo(doorCX, doorTop).lineTo(doorCX, doorTop + doorH);
      }
    }
  ...
  _moto(g, highlight) {
    ...
    g.moveTo(rwx, wCy).lineTo(hipX, hipY);
    g.moveTo(hipX, hipY).lineTo(bar.x, bar.y);
    g.moveTo(rwx, wCy).lineTo(bar.x - d * 8 * bs, bar.y + 5 * bs);
    ...
    g.moveTo(hipX - d * 10 * bs, hipY + 1 * bs).lineTo(hipX + d * 3 * bs, hipY - 1 * bs);
    ...
    g.moveTo(bar.x - d * 4 * bs, bar.y + 2 * bs).lineTo(bar.x + d * 5 * bs, bar.y - 3 * bs);

    // 排气管
    g.lineStyle(Math.max(0.8, bs * 4), 0x888888, 0.6);
    g.moveTo(hipX - d * 6 * bs, hipY + 6 * bs).lineTo(rwx + d * wR * 0.6, wCy + wR * 0.4);

    // 踏板块
    g.lineStyle(Math.max(1.5, bs * 4), 0x2a2a2a, 1);
    g.moveTo(footF.x - d * 2 * bs, footF.y).lineTo(footF.x + d * 3 * bs, footF.y);
    g.moveTo(footR.x - d * 2 * bs, footR.y).lineTo(footR.x + d * 3 * bs, footR.y);
