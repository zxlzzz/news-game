...
class MovementAudit {
  ...
  _entry(npc) {
    ...
    if (!this._counts.has(id)) this._counts.set(id, { probe_steer: 0, slide_steer: 0, blocked_contact: 0, stuck: 0, dir_mismatch: 0, vel0_walk: 0, departing_orphan: 0 });
  ...
  dump(npcs) {
    const rows = [];
    let gProbe = 0, gSlide = 0, gBlocked = 0, gStuck = 0, gDirMismatch = 0, gSpeed0Walk = 0, gDepartingOrphan = 0;
