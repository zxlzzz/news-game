...
export class SceneRenderer {
  ...
  _drawGround() {
    const g = this.bg;
    g.lineStyle(0); g.beginFill(GRAY_FAR_PAVE, 1); g.drawRect(0, BUILDING_BASE_Y, WORLD_WIDTH, FAR_Y - BUILDING_BASE_Y); g.endFill();
    g.lineStyle(0); g.beginFill(GRAY_ROAD, 1); g.drawRect(0, FAR_Y, WORLD_WIDTH, NEAR_Y - FAR_Y); g.endFill();
    g.lineStyle(0); g.beginFill(GRAY_NEAR_PAVE, 1); g.drawRect(0, NEAR_Y, WORLD_WIDTH, BIKE_LANE_NEAR_BOTTOM - NEAR_Y); g.endFill();
    g.lineStyle(0); g.beginFill(0xcacaca, 1); g.drawRect(0, PARK_TOP, WORLD_WIDTH, WORLD_HEIGHT - PARK_TOP); g.endFill();
  ...
  _drawBusStops() {
    ...
    const drawSign = (px, py) => {
      ...
      g.lineStyle(0); g.beginFill(0x1a44aa, 1); g.drawRect(sx, py, sw, sh); g.endFill();
      g.lineStyle(1, 0x0a0820, 1); g.drawRect(sx, py, sw, sh); g.lineStyle(0);
      g.lineStyle(0); g.beginFill(0xeaeaf0, 1); g.drawRect(sx + 2, py + 2, sw - 4, sh - 4); g.endFill();
      g.lineStyle(0); g.beginFill(0x2255bb, 0.85); g.drawRect(sx + 3, py + 3, sw - 6, 4); g.endFill();
      g.lineStyle(0); g.beginFill(0x303050, 0.55);
  ...
  _drawFarBusStop(g, stop, ft, fy, drawSign) {
    ...
    g.lineStyle(0); g.beginFill(GRAY_ROAD, 1); g.drawRect(bx0, fy - BAY_D, BAY_W, BAY_D); g.endFill();
    g.lineStyle(0); g.beginFill(0xd8d8d8, 1);
    g.drawRect(bx0 - 3, fy - BAY_D - 3, 3,         BAY_D + 3);
    g.drawRect(bx1,     fy - BAY_D - 3, 3,         BAY_D + 3);
    g.drawRect(bx0 - 3, fy - BAY_D - 3, BAY_W + 6, 3);
    g.endFill();
    g.lineStyle(0); g.beginFill(0xb2b2b0, 1); g.drawRect(bx0, fy - BAY_D - 2, BAY_W, 2); g.endFill();
    ...
    g.lineStyle(0); g.beginFill(0x565654, 1); g.drawRect(sx - benchHalf, benchY, BENCH_W, 4 * sf); g.endFill();
    g.lineStyle(0.8 * sf, 0x181818, 0.7); g.drawRect(sx - benchHalf, benchY, BENCH_W, 4 * sf); g.lineStyle(0);
    g.lineStyle(1.5 * sf, 0x303030, 0.9);
    g.moveTo(sx - benchHalf + 10, benchY + 4 * sf); g.lineTo(sx - benchHalf + 10, benchY + 8 * sf);
    g.moveTo(sx + benchHalf - 10, benchY + 4 * sf); g.lineTo(sx + benchHalf - 10, benchY + 8 * sf);
  ...
  _drawNearBusStop(g, stop, ny, drawSign) {
    ...
    g.lineStyle(0); g.beginFill(GRAY_ROAD, 1); g.drawRect(bx0, ny, BAY_W, BAY_D); g.endFill();
    g.lineStyle(0); g.beginFill(0xd8d8d8, 1);
    g.drawRect(bx0 - 3, ny,         3,         BAY_D + 3);
    g.drawRect(bx1,     ny,         3,         BAY_D + 3);
    g.drawRect(bx0 - 3, ny + BAY_D, BAY_W + 6, 3);
    g.endFill();
    g.lineStyle(0); g.beginFill(0xb2b2b0, 1); g.drawRect(bx0, ny + BAY_D, BAY_W, 2); g.endFill();

    // 顶棚 + 柱子已移到 busstop-roof PropEntity（参与 Y 排序）；这里只保留地面站台 / 长椅 / 标牌。
    const sn     = depthScale(ny);
    const benchY = ny + 32;
    const benchHalf = BENCH_W / 2;
    g.lineStyle(0); g.beginFill(0x565654, 1); g.drawRect(sx - benchHalf, benchY, BENCH_W, 4 * sn); g.endFill();
    g.lineStyle(0.8 * sn, 0x181818, 0.7); g.drawRect(sx - benchHalf, benchY, BENCH_W, 4 * sn); g.lineStyle(0);
    g.lineStyle(1.5 * sn, 0x303030, 0.9);
    g.moveTo(sx - benchHalf + 12, benchY + 4 * sn); g.lineTo(sx - benchHalf + 12, benchY + 9 * sn);
    g.moveTo(sx + benchHalf - 12, benchY + 4 * sn); g.lineTo(sx + benchHalf - 12, benchY + 9 * sn);
    ...
    g.moveTo(poleX, poleBy); g.lineTo(poleX, poleTy);
    drawSign(poleX, poleTy);
  }
  ...
  _drawChessPlaza(g) {
    ...
    g.lineStyle(0); g.beginFill(0xebebeb, 0.4); g.drawEllipse(cx, cy, rx, ry); g.endFill();
    g.lineStyle(1, 0xcccccc, 0.9); g.drawEllipse(cx, cy, rx, ry); g.lineStyle(0);
  ...
  _drawMiniPark(g) {
    ...
    g.lineStyle(0); g.beginFill(0xe8e8e8, 1); g.drawEllipse(cx, cy, rx, ry); g.endFill();
  ...
  _drawSky() {
    const g = this.sky;
    g.clear();
    g.lineStyle(0); g.beginFill(GRAY_SKY, 1); g.drawRect(-300, 0, WORLD_WIDTH + 600, SKY_Y); g.endFill();
  ...
  _drawFarSkyline(g) {
    ...
    for (let i = 0; i < 48; i++) {
      ...
      g.lineStyle(0); g.beginFill(0xf1f1f1, 1); g.drawRect(bx, base - bh, bw, bh); g.endFill();
    }
    for (let i = 0; i < 32; i++) {
      const bx = i * 70 - 20 + seed(i + 50) * 28;
      const bw = 44 + seed(i + 60) * 36;
      const bh = 110 + seed(i + 70) * 70;
      g.lineStyle(0); g.beginFill(0xe6e6e6, 1); g.drawRect(bx, base - bh, bw, bh); g.endFill();
      g.lineStyle(0.5, 0xd6d6d6, 0.5); g.drawRect(bx, base - bh, bw, bh); g.lineStyle(0);
  ...
  _drawClouds(g) {
    for (const c of (this.layout.clouds || [])) {
      ...
      g.lineStyle(0); g.beginFill(0xffffff, 0.92);
      g.drawEllipse(cx,          cy,         35 * s, 13 * s);
      g.drawEllipse(cx - 28 * s, cy + 6 * s, 22 * s, 10 * s);
      g.drawEllipse(cx + 30 * s, cy + 5 * s, 24 * s, 10 * s);
      g.endFill();
      g.lineStyle(0.8, 0xd2d2d2, 0.6); g.drawEllipse(cx, cy, 35 * s, 13 * s); g.lineStyle(0);
    }
  }
  ...
  _drawParkPlaza(g) {
    ...
    g.lineStyle(0); g.beginFill(0xdedede, 1); g.drawRect(0, top + 4, WORLD_WIDTH, 22); g.endFill();
  ...
  _drawRoadMarkings(g) {
    g.lineStyle(0); g.beginFill(GRAY_CURB, 1); g.drawRect(0, FAR_Y - 3, WORLD_WIDTH, 3); g.endFill();
    ...
    g.lineStyle(0); g.beginFill(0x888888, 0.85); g.drawRect(0, FAR_Y, WORLD_WIDTH, 4); g.endFill();

    g.lineStyle(0); g.beginFill(0xd8d8d8, 1); g.drawRect(0, NEAR_Y, WORLD_WIDTH, 4); g.endFill();
  ...
  _drawRoadPatches(g) {
    ...
    g.lineStyle(0); g.beginFill(0x8a8a8a, 0.4);
    for (let i = 0; i < 3; i++) {
  ...
  _drawCrosswalk(g, cx) {
    ...
    g.lineStyle(0); g.beginFill(0xffffff, 0.68);
