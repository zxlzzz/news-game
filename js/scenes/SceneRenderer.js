...
export class SceneRenderer {
  ...
  _drawGround() {
    ...
    g.moveTo(0, BIKE_LANE_FAR_TOP).lineTo(WORLD_WIDTH, BIKE_LANE_FAR_TOP);
    g.moveTo(0, BIKE_LANE_NEAR_BOTTOM).lineTo(WORLD_WIDTH, BIKE_LANE_NEAR_BOTTOM);
  ...
  _drawChessPlaza(g) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      g.moveTo(cx, cy).lineTo(cx + Math.cos(a) * rx, cy + Math.sin(a) * ry);
    }
  }
  ...
  _drawMiniPark(g) {
    ...
    for (let i = 0; i < 28; i++) {
      ...
      g.moveTo(gx, gy).lineTo(gx, gy - 3);
      g.moveTo(gx, gy - 1.5).lineTo(gx - 1.5, gy - 3.5);
      g.moveTo(gx, gy - 1.5).lineTo(gx + 1.5, gy - 3.5);
    }
  }
  ...
  _drawFarSkyline(g) {
    ...
    for (let i = 0; i < 32; i++) {
      ...
      for (let k = 1; k < 3; k++) { const lx = bx + bw * k / 3; g.moveTo(lx, base - bh + 6).lineTo(lx, base - 4); }
    }
  }
  ...
  _drawParkPlaza(g) {
    ...
    g.moveTo(0, top + 4).lineTo(WORLD_WIDTH, top + 4);
    g.moveTo(0, top + 26).lineTo(WORLD_WIDTH, top + 26);
  }
  ...
  _drawRoadMarkings(g) {
    ...
    g.moveTo(0, FAR_Y - 3).lineTo(WORLD_WIDTH, FAR_Y - 3);
    g.moveTo(0, FAR_Y).lineTo(WORLD_WIDTH, FAR_Y);
    g.lineStyle(0);
    ...
    g.moveTo(0, NEAR_Y + 4).lineTo(WORLD_WIDTH, NEAR_Y + 4);
    ...
    for (let x = 0; x < WORLD_WIDTH; x += spacing) {
      g.moveTo(x, midY).lineTo(x + length, midY);
    }
