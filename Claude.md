...
- [ ] **阶段1：分带重新比例化（地基）**
  ...
  - CHESS_PLAZA 移入新公园
- [ ] **阶段2：建筑改造（混合用途连续街墙 + 单行道缺口）**
  - 4–5 层：底层店铺（超市/咖啡/银行/便利店/健身房/书店/水果店/汉堡店/诊所），不同招牌门面区分
  - 上层居民阳台：绿植/晾衣绳/空调外机（灰度）
  - 连续街墙，少数几处留约一个建筑宽的缺口当单行道（仅露出地面的简单缺口，不画复杂巷子）
  - 重写 scene.json buildings + BuildingEntity 立面/招牌类型
- [ ] **阶段3：公园内容**
...
- [ ] **阶段5：NPC 密度与自由漫游**
  - 行人在公园/广场矩形内自由二维走动（走向随机目标，按 Y 缩放排序）
  - 提高 NPC 数量；混合行为：走/坐/遛狗/骑车/过马路（过马路可化简）
