...
> 用户根据一张实景街拍写的完整规格，分阶段执行。**已锁定的决策**：
> - **保留黑白灰画风**（忽略规格里的彩色要求，依旧纯灰度）
> - **行人改为公园/广场内自由二维漫游**（按 Y 缩放排序），不再固定横线
> - **保留 ~2000 宽、可左右滚动的世界**

### 目标分带（屏幕高 500，轻微俯视 ~30°，Y 越大越近）

```
远景  0–100      (1/5)   天空白云（纯装饰，已做视差层）
中景  100–210            连续街墙：4–5 层混合用途楼（底层店铺+上层居民阳台）
      210–250            短前街（人行道）：行人/树/垃圾桶/长椅
近景  250–333    (1/3)   双行道：公交/出租/私家车/摩托
      333–500    (2/3)   公园广场：喷泉/滑梯/野餐草地/小摊/长椅/树，NPC 主活动区
```

### 分阶段 TODO

- [ ] **阶段1：分带重新比例化（地基）**
  - SceneConfig 新分带常量（SKY_Y=100 / BUILDING_BASE_Y=210 / FAR_Y=250 / NEAR_Y=333）
  - 加宽道路成双行道；公园占下半屏 2/3
  - _drawTrees 等硬编码 Y、scene.json 道具 Y 适配新分带（NPC 走常量自动跟随）
  - CHESS_PLAZA 移入新公园
- [x] **阶段2：建筑改造（6 种原型 + 老街区质感）**
  - 6 原型：resi 居民楼(主,6–8层,浅灰,密阳台/空调/晾衣,底层便利店或小餐馆) /
    oldmix 老旧商住(4–6层窄,防盗网+卷帘门+不规则招牌+脏纹+拥挤) /
    modern 现代商业(玻璃幕,3–5层,干净,咖啡/书店/健身) /
    clinic 社区诊所(白灰,规则窗,门口雨棚+十字) /
    convenience 街角便利店(矮,大横招牌,玻璃门,门口货箱+自动售货机) /
    bookstore 老式书店(木牌,窄门,橱窗书脊,略旧)
  - 高度/窗户/招牌均不统一；屋顶可有水箱/太阳能板/广告牌/空调/天线
  - 连续街墙(19 栋,缺口 x≈540–650、1380–1490 作侧路)；背景天际线调更浅更简
  - 楼可做高(允许遮挡天空)；分布 resi7/oldmix5/modern3/便利店2/书店1/诊所1
- [x] **阶段3：公园内容**
  - 新 PropEntity 类型：fountain 喷泉 / slide 儿童滑梯 / picnic 野餐垫 / stall 小摊贩亭
  - 大公园里再划"小公园"游园区 MINI_PARK（带环园小径+灌木边界，喷泉居中、滑梯/野餐垫在其内）
  - 公园道具放大 2–3×；路灯加高约 1.7×；NPC 全局缩小 30%（farScale 0.182 / nearScale 0.434）
- [x] **阶段4：车辆**
  - 新 VehicleEntity（灰度侧视，按 Y 缩放排序，双行道循环行驶）：car/taxi/bus/moto
  - 上行带向右、下行带向左；保留原自行车/电动车火柴骑手
- [~] **阶段5：NPC 密度与自由漫游**（核心已完成）
  - [x] 行人在公园矩形内自由二维走动（BehaviorManager 转向随机目标，按 Y 缩放排序）
  - [x] 提高 NPC 数量（公园 12 名漫游者 + 前人行道 3 名 + 横穿者）
  - [ ] 待办：遛狗者/运动者也接入漫游；骑车者/过马路进一步丰富

### 注意
- NPC 生成位置均由 SceneConfig 常量派生 → 改常量即自动跟随；scene.json 道具与 _drawTrees 为硬编码 Y，需手动适配。
- 取景框按世界坐标判定捕获，装饰背景才可做视差。
