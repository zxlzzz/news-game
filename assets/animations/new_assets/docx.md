...
## 多帧 sub-event overlay 格式（SE-2，role 名数据驱动见 SE-3）

`overlay` + `participants` 的 duet clip（`push` / `give_item` / `handshake` / `point_at`
及新增的 `shakehand` 一类）现支持多帧关键帧序列，不再局限于单帧静态叠加：

- `participants[]`：每项 `{role, dx?}`。`role` 是任意字符串，就是 keyframe 里对应
  子对象的 key——**不会**被翻译成固定的 `a`/`b`（SE-3 起 `TalkActivity.js` 按
  `PoseCacheBuilder.decodeSubEvent()` 返回的 `roles` 数组取实际名字：`roles[0]` 绑
  `TalkActivity.a`、`roles[1]` 绑 `TalkActivity.b`，只按位置对应，不认字面量；
  `push`/`give_item`/`handshake`/`point_at` 四个既有 clip 恰好把 role 写成 `a`/`b`，
  纯属巧合沿用，不是硬性要求，新 clip 可以叫 `giver`/`receiver` 之类的语义名）。
  `dx` 是该角色相对 `role 0`（隐式 `dx=0`）的设计站位间距，缺省回退编辑器默认值
  70px（`sth/stick-puppet/js/app.js` `DUET_DEFAULT_DX`）。两个角色的 `dx` 之差就是
  `decodeSubEvent()` 算出的 `designGap`——运行时 `TalkActivity` 在 reach 阶段把两个
  NPC 的 x 收拢到这个间距、release 阶段再还原回原位，中点不变。
- `keyframes[]`：每帧 `{[role]: {...}, dur?}`，`role` 用 `participants[]` 里写的原始
  名字做 key（不是固定 `a`/`b`），内容和单帧格式一样是相对 defaultPose 的关节 delta。
  多帧即让这条 duet clip 变成一段可播放序列，而不是一个瞬间姿势。
- `reach` / `release`（可选，clip 顶层，秒）：进入/退出这段动作的过渡时长。省略时
  `TalkActivity.js` 统一回退 0.4s（SE-3 起不再有按事件类型区分的硬编码值，想要不同
  的过渡节奏直接在 JSON 里写这两个字段即可，不用改代码）。
- `dur`（可选，每帧独立，秒）：该帧播放时长。省略时统一回退 0.3s（SE-3 起单帧/多帧
  用同一个回退值，不再有单帧特例）。
...
**与单帧旧格式的兼容**：旧格式就是"只有 1 帧、没有 `sustain`/`reach`/`release` 字段"
的特例——`decodeSubEvent()` 不区分新旧，统一解码成 `frames` 数组；`push`/`give_item`/
`handshake`/`point_at` 四个既有 clip 不用改 JSON 就能继续被选中、播放，reach→play→
release 三段式照常走完，但过渡/停留时长不再是各事件类型手调过的数值，而是统一走
上面两条的默认回退（0.4s / 0.3s）——如果要恢复某个事件原本更长的停留手感，直接给
对应 JSON 补 `reach`/`release`/`keyframes[0].dur` 字段，不需要碰 `TalkActivity.js`。
