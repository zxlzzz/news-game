...
| 批次 B-①a（AttachmentDefs + SimpleProp + chain 通道） | `AttachmentDefs.js` 纯数据声明表（snack/broom）；`SimpleProp.js` rect/circle/line 几何道具渲染；`NpcPropManager` default 分支查 AttachmentDefs + `_chain_` 前缀 modifier 解析（ModifierLayer 豁免，无需改） | ✅ 已落地 | `js/behavior/data/AttachmentDefs.js`；`js/npc/props/SimpleProp.js`；`js/npc/props/NpcPropManager.js` |
