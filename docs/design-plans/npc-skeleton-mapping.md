...
以下图片位于 `C:/Users/Hsinlung/Desktop/npc_skeleton/npc_skeleton/`。调参 HTML 内置 6 条动作、660 帧；本轮默认参数检查未发现无效坐标，骨段长度与缩放后踝目标误差仅浮点精度。此检查不等于对全部 63 条动作逐帧验证，也不是通用防穿模或手掌接触保证。
